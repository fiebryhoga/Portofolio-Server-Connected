...
const Navbar = () => {
  ...
  return (
    <div className="w-full justify-between pr-6 sm:pr-12 md:pr-0 md:justify-normal z-50 fixed flex flex-row md:gap-16 bg-transparent py-5 border-b border-b-gray-200 border-opacity-15 items-center backdrop-blur-lg">
      <Link href="/" passHref>
      ...
      <nav className=" hidden md:flex w-full flex-row items-center gap-40">
        <ul className="lg:px-44 md:px-16 flex flex-row justify-around w-full tracking-wide text-sm text-[#c3e7e5]">
          {navList.map((item) => (
            <li key={item.nav} onClick={() => handleScroll(item.id)}>
      ...
      >
        <nav className="w-full gap-8">
          <ul className="flex flex-col mt-5 w-full">
            ...
            {navList.map((item) => (
              <li key={item.nav} className="w-full">
                <a
                  href={`#${item.id}`}
                  className="w-full py-4 px-4 hover:bg-gray-200 block"
                  onClick={() => {
